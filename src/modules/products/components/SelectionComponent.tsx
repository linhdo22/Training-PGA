...
function SelectionComponent(props: Props) {
    ...
    return (
        <div
            className="filter-custom-selection"
            tabIndex={0}
            onBlur={handleBlur}
        >
            <div
            ...
            >
                {selectedValue
                    ? list.find((option) => option.value == selectedValue)
                          ?.label
                    : title}
                <i className="fa-solid fa-angle-down"></i>
            </div>
            <div
                className={`filter-custom-selection__content ${
                    expand ? '' : 'd-none'
                }`}
                style={{ minWidth: '100%' }}
            >
                {list.map((value: SelectOption, index) => {
                    return (
                        <div
                            key={index}
                            className={`${
                                value.value == selectedValue ? 'selected' : ''
                            }`}
